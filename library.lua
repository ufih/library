-- NexusLib v4.3.6 Enhanced Drawing UI Library for Roblox
-- Features:
-- Modern dark theme with multiple color schemes
-- Draggable windows, watermark, and keybind list
-- Full element set: Toggle, Slider, Button, Dropdown, Keybind, ColorPicker, Textbox, Label
-- Notification system with animations
-- Config save/load system
-- Real-time theme and accent color switching
-- Tabs positioned on RIGHT side of topbar
-- 
-- FIXES IN v4.3.6:
-- 1. Section sidebar button highlight now updates immediately on click (no double-click needed)
-- 2. Unload function now properly cleans up everything
----
...
function library.watermark:SetEnabled(state)
    ...
    if library.watermark.initialized then
...
function library.keybindList:SetEnabled(state)
    ...
    if library.keybindList.initialized then
...
local function createDrawing(class, properties)
    if library.unloaded then return { Remove = function() end, Visible = false } end
...
function library:Notify(config)
    ...
    task.spawn(function()
        while currentX > targetX and not library.unloaded do
        ...
        local elapsed = 0
        while elapsed < duration and not library.unloaded do
        ...
        while currentX < outTarget and not library.unloaded do
...
function library:New(config)
    ...
    function window:Page(config)
        ...
        function page:Section(config)
            ...
            -- FIX: Update visual state based on whether this section is current
            function sectionBtn:SetVisible(state)
                local isCurrent = section == page.currentSection
                pcall(function() sectionBtn.objects.accent.Visible = state and isCurrent end)
                pcall(function() sectionBtn.objects.text.Visible = state end)
                pcall(function() sectionBtn.objects.text.Color = isCurrent and Color3.new(1, 1, 1) or library.theme.dimtext end)
            end

            -- FIX: New function to just update the highlight without changing visibility
            function sectionBtn:UpdateHighlight()
                local isCurrent = section == page.currentSection
                pcall(function() sectionBtn.objects.accent.Visible = page.visible and isCurrent end)
                pcall(function() sectionBtn.objects.text.Color = isCurrent and Color3.new(1, 1, 1) or library.theme.dimtext end)
            end
            ...
            -- FIX: Set currentSection BEFORE updating visuals so highlights work correctly
            function section:Show()
                window:ClosePopups()
                -- Set current section FIRST
                page.currentSection = section
                -- Then hide all sections
                for _, s in pairs(page.sections) do
                    if s ~= section and s.SetVisible then
                        s:SetVisible(false)
                    end
                end
                -- Show this section
            ...
            table.insert(library.connections, UserInputService.InputBegan:Connect(function(input)
                ...
                if input.UserInputType == Enum.UserInputType.MouseButton1 and library.open and page.visible then
                    local p = window.pos
                    local btnX = p.X + 5
                    local btnYPos = p.Y + 32 + sectionBtn.yOffset
                    if isMouseOver(btnX, btnYPos, 116, 20) then
                        section:Show()
                    end
                end
            end))
            ...
            function section:Toggle(config)
                ...
                local toggle = { value = default, objects = {}, flag = flag }
                ...
                function toggle:UpdatePositions()
                    local p = window.pos
                    pcall(function() toggle.objects.label.Position = p + Vector2.new(baseX + 10, contentY + toggle.yOffset) end)
                    pcall(function() toggle.objects.outline.Position = p + Vector2.new(baseX + contentWidth - 20, contentY + toggle.yOffset + 2) end)
                    pcall(function() toggle.objects.fill.Position = p + Vector2.new(baseX + contentWidth - 19, contentY + toggle.yOffset + 3) end)
                end
            ...
            function section:Slider(config)
                ...
                local slider = { value = default, min = min, max = max, dragging = false, objects = {}, flag = flag }
                ...
                function slider:Set(value)
                    ...
                    pcall(function()
                        ...
                        slider.objects.value.Position = window.pos + Vector2.new(
                            baseX + contentWidth - 10 - textWidth, contentY + slider.yOffset
                        )
                    end)
                    pcall(function() callback(value) end)
                end
                ...
                function slider:UpdatePositions()
                    local p = window.pos
                    pcall(function() slider.objects.label.Position = p + Vector2.new(baseX + 10, contentY + slider.yOffset) end)
                    pcall(function()
                        local textWidth = getTextBounds(slider.objects.value.Text, 13).X
                        slider.objects.value.Position = p + Vector2.new(
                            baseX + contentWidth - 10 - textWidth, contentY + slider.yOffset
                        )
                    end)
                    pcall(function() slider.objects.bg.Position = p + Vector2.new(baseX + 10, contentY + slider.yOffset + 18) end)
                    pcall(function() slider.objects.fill.Position = p + Vector2.new(baseX + 10, contentY + slider.yOffset + 18) end)
                end
            ...
            function section:Button(config)
                ...
                button.objects.label = createDrawing("Text", {
                    ...
                    Outline = true,
                    OutlineColor = Color3.new(0, 0, 0),
                    Position = window.pos + Vector2.new(
                        baseX + 10 + ((contentWidth - 20) / 2) - (textWidth / 2),
                        contentY + offset + 4
                    ),
                    Visible = section.visible,
                    ZIndex = 12
                ...
                function button:UpdatePositions()
                    local p = window.pos
                    pcall(function() button.objects.outline.Position = p + Vector2.new(baseX + 10, contentY + button.yOffset) end)
                    pcall(function() button.objects.bg.Position = p + Vector2.new(baseX + 11, contentY + button.yOffset + 1) end)
                    pcall(function() button.objects.label.Position = p + Vector2.new(
                        baseX + 10 + ((contentWidth - 20) / 2) - (textWidth / 2),
                        contentY + button.yOffset + 4
                    ) end)
                end
            ...
            function section:Dropdown(config)
                ...
                local dropdown = { value = default, options = options, open = false, objects = {}, optionObjects = {}, flag = flag }
                ...
                function dropdown:UpdatePositions()
                    local p = window.pos
                    pcall(function() dropdown.objects.label.Position = p + Vector2.new(baseX + 10, contentY + dropdown.yOffset) end)
                    pcall(function() dropdown.objects.outline.Position = p + Vector2.new(baseX + 10, contentY + dropdown.yOffset + 18) end)
                    pcall(function() dropdown.objects.bg.Position = p + Vector2.new(baseX + 11, contentY + dropdown.yOffset + 19) end)
                    pcall(function() dropdown.objects.selected.Position = p + Vector2.new(baseX + 16, contentY + dropdown.yOffset + 22) end)
                    pcall(function() dropdown.objects.arrow.Position = p + Vector2.new(baseX + contentWidth - 20, contentY + dropdown.yOffset + 22) end)
                end
                ...
                function dropdown:SetVisible(state)
                    ...
                    if not state and dropdown.open then dropdown:Close() end
                end
            ...
            function section:Keybind(config)
                ...
                local keybind = { value = default, listening = false, objects = {}, flag = flag }
                ...
                function keybind:UpdatePositions()
                    local p = window.pos
                    pcall(function() keybind.objects.label.Position = p + Vector2.new(baseX + 10, contentY + keybind.yOffset) end)
                    pcall(function()
                        local kText = "[" .. getKeyName(keybind.value) .. "]"
                        keybind.objects.key.Position = p + Vector2.new(
                            baseX + contentWidth - 10 - getTextBounds(kText, 13).X,
                            contentY + keybind.yOffset
                        )
                    end)
                end
                ...
                table.insert(library.connections, UserInputService.InputBegan:Connect(function(input)
                    ...
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and library.open and section.visible then
                        local p = window.pos
                        local kText = "[" .. getKeyName(keybind.value) .. "]"
                        local keyWidth = getTextBounds(kText, 13).X
            ...
            function section:ColorPicker(config)
                ...
                local default = config.default or library.accent
                local flag = config.flag or name
                local side = config.side or "left"
                local callback = config.callback or function() end
                ...
                local colorpicker = { value = default, open = false, objects = {}, pickerObjects = {}, flag = flag, hue = 0, sat = 1, val = 1 }

                local h, s, v = rgbToHsv(default.R * 255, default.G * 255, default.B * 255)
                colorpicker.hue, colorpicker.sat, colorpicker.val = h, s, v
                ...
                registerTheme(colorpicker.objects.label, "Color", "text")

                colorpicker.objects.outline = createDrawing("Square", {
                    Size = Vector2.new(20, 12),
                    Position = window.pos + Vector2.new(baseX + contentWidth - 30, contentY + offset),
                    Color = library.theme.outline,
                    Filled = true,
                    Visible = section.visible,
                    ZIndex = 10
                })
                registerTheme(colorpicker.objects.outline, "Color", "outline")

                colorpicker.objects.preview = createDrawing("Square", {
                    Size = Vector2.new(18, 10),
                    Position = window.pos + Vector2.new(baseX + contentWidth - 29, contentY + offset + 1),
                    Color = default,
                    Filled = true,
                    Visible = section.visible,
                    ZIndex = 11
                ...
                function colorpicker:UpdateColor()
                    local r, g, b = hsvToRgb(colorpicker.hue, colorpicker.sat, colorpicker.val)
                    local newColor = Color3.fromRGB(r, g, b)
                    colorpicker.value = newColor
                    library.flags[flag] = newColor
                    pcall(function() colorpicker.objects.preview.Color = newColor end)
                    if colorpicker.pickerObjects.previewBox then
                        pcall(function() colorpicker.pickerObjects.previewBox.Color = newColor end)
                    end
                    if colorpicker.pickerObjects.svBox then
                        pcall(function() colorpicker.pickerObjects.svBox.Color = Color3.fromHSV(colorpicker.hue, 1, 1) end)
                    end
                    pcall(function() callback(newColor) end)
                end

                function colorpicker:Open()
                    if window.activeColorPicker and window.activeColorPicker ~= colorpicker then
                        window.activeColorPicker:Close()
                    end
                    ...
                    local p = window.pos
                    local pickerX = baseX + contentWidth - 170
                    local pickerY = contentY + offset + 18
                    colorpicker.pickerX = pickerX
                    colorpicker.pickerY = pickerY

                    colorpicker.pickerObjects.outline = createDrawing("Square", {
                        Size = Vector2.new(164, 120),
                        Position = p + Vector2.new(pickerX, pickerY),
                        Color = library.theme.outline,
                        Filled = true,
                        Visible = true,
                        ZIndex = 50
                    ...
                    colorpicker.pickerObjects.bg = createDrawing("Square", {
                        Size = Vector2.new(162, 118),
                        Position = p + Vector2.new(pickerX + 1, pickerY + 1),
                        Color = library.theme.background,
                        Filled = true,
                        Visible = true,
                        ZIndex = 51
                    ...
                    colorpicker.pickerObjects.svBox = createDrawing("Square", {
                        Size = Vector2.new(130, 108),
                        Position = p + Vector2.new(pickerX + 6, pickerY + 6),
                        Color = Color3.fromHSV(colorpicker.hue, 1, 1),
                        Filled = true,
                        Visible = true,
                        ZIndex = 52
                    })

                    colorpicker.pickerObjects.previewBox = createDrawing("Square", {
                        Size = Vector2.new(15, 15),
                        Position = p + Vector2.new(pickerX + 141, pickerY + 99),
                ...
                function colorpicker:Close()
                    ...
                    for _, obj in pairs(colorpicker.pickerObjects) do
                        pcall(function() removeDrawing(obj) end)
                    end
                ...
                function colorpicker:UpdatePositions()
                    local p = window.pos
                    pcall(function() colorpicker.objects.label.Position = p + Vector2.new(baseX + 10, contentY + colorpicker.yOffset) end)
                    pcall(function() colorpicker.objects.outline.Position = p + Vector2.new(baseX + contentWidth - 30, contentY + colorpicker.yOffset) end)
                    pcall(function() colorpicker.objects.preview.Position = p + Vector2.new(baseX + contentWidth - 29, contentY + colorpicker.yOffset + 1) end)
                end
                ...
                function colorpicker:SetVisible(state)
                    ...
                    if not state and colorpicker.open then colorpicker:Close() end
                end

                colorpicker.yOffset = offset
                ...
                table.insert(library.connections, UserInputService.InputBegan:Connect(function(input)
                    ...
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and library.open then
                        ...
                        if colorpicker.open then
                            local svX = p.X + colorpicker.pickerX + 6
                            local svY = p.Y + colorpicker.pickerY + 6
                            if isMouseOver(svX, svY, 130, 108) then
                                draggingSV = true
                                return
                            end
                            local hueX = p.X + colorpicker.pickerX + 141
                            local hueY = p.Y + colorpicker.pickerY + 6
                            if isMouseOver(hueX, hueY, 15, 108) then
                                draggingHue = true
                                return
                            end
                            colorpicker:Close()
                        elseif section.visible then
                            local previewX = p.X + baseX + contentWidth - 30
                            local previewY = p.Y + contentY + colorpicker.yOffset
                            if isMouseOver(previewX, previewY, 20, 12) then
                                colorpicker:Open()
                            end
                        end
                    end
                    ...
                    if colorpicker.open then
                        ...
                        if success then
                            ...
                            if draggingSV then
                                local svX = p.X + colorpicker.pickerX + 6
                                local svY = p.Y + colorpicker.pickerY + 6
                                local relX = math.clamp(mouse.X - svX, 0, 130) / 130
                                local relY = math.clamp(mouse.Y - svY, 0, 108) / 108
                                colorpicker.sat = relX
                                colorpicker.val = 1 - relY
                                colorpicker:UpdateColor()
                            elseif draggingHue then
                                local hueY = p.Y + colorpicker.pickerY + 6
                                local relY = math.clamp(mouse.Y - hueY, 0, 108) / 108
                ...
                if isLeft then
                    section.leftOffset = section.leftOffset + 22
                    table.insert(section.leftElements, colorpicker)
                else
                    section.rightOffset = section.rightOffset + 22
                    table.insert(section.rightElements, colorpicker)
                end
            ...
            function section:Textbox(config)
                ...
                local textbox = { value = default, focused = false, objects = {}, flag = flag }
                ...
                registerTheme(textbox.objects.label, "Color", "text")
                ...
                function textbox:UpdatePositions()
                    local p = window.pos
                    pcall(function() textbox.objects.label.Position = p + Vector2.new(baseX + 10, contentY + textbox.yOffset) end)
                    pcall(function() textbox.objects.outline.Position = p + Vector2.new(baseX + 10, contentY + textbox.yOffset + 18) end)
                    pcall(function() textbox.objects.bg.Position = p + Vector2.new(baseX + 11, contentY + textbox.yOffset + 19) end)
                    pcall(function() textbox.objects.text.Position = p + Vector2.new(baseX + 16, contentY + textbox.yOffset + 22) end)
                end
                ...
                table.insert(library.connections, UserInputService.InputBegan:Connect(function(input)
                    ...
                    if input.UserInputType == Enum.UserInputType.MouseButton1 and library.open and section.visible then
                        local p = window.pos
                        local boxX = p.X + baseX + 10
                        local boxY = p.Y + contentY + textbox.yOffset + 18
                        if isMouseOver(boxX, boxY, contentWidth - 20, 22) then
                            ...
                            pcall(function()
                                ...
                                textbox.objects.text.Color = textbox.value ~= "" and library.theme.text or library.theme.dimtext
                            end)
                        end
                    end
                    ...
                    if textbox.focused and input.UserInputType == Enum.UserInputType.Keyboard then
                        ...
                        elseif input.KeyCode == Enum.KeyCode.Return then
                            ...
                            pcall(function() callback(textbox.value) end)
                        else
                            local keyString = UserInputService:GetStringForKeyCode(input.KeyCode)
                            if keyString and #keyString == 1 then
                                ...
                                textbox.value = textbox.value .. keyString
                            end
                        end
                        pcall(function() textbox.objects.text.Text = textbox.value end)
                ...
                if isLeft then
                    section.leftOffset = section.leftOffset + 48
                    table.insert(section.leftElements, textbox)
                else
                    section.rightOffset = section.rightOffset + 48
                    table.insert(section.rightElements, textbox)
                end
            ...
            function section:Label(config)
                ...
                function label:UpdatePositions()
                    local p = window.pos
                    pcall(function() label.objects.text.Position = p + Vector2.new(baseX + 10, contentY + label.yOffset) end)
                end
    ...
    table.insert(library.connections, UserInputService.InputBegan:Connect(function(input)
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 and library.open then
            local p = window.pos
            for _, page in pairs(window.pages) do
                local tX = p.X + page.tabX
                local tY = p.Y + 4
                if isMouseOver(tX, tY, page.tabWidth, 21) then
                    page:Show()
                    break
                end
            end
    ...
    function window:Unload()
        ...
        if library.watermark.initialized then
            ...
            library.watermark.initialized = false
            library.watermark.enabled = false
        end
        ...
        if library.keybindList.initialized then
            ...
            library.keybindList.initialized = false
            library.keybindList.enabled = false
        end
